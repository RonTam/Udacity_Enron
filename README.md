...
Before we go into more detail about these numbers, it's first prudent to explain how the numbers were achieved.  We relied on the algorithm given by tester.py, which uses a Stratified Shuffle Split for Cross Validation.  In machine learning, we typically train a model using observed, labeled data, but this can often overfit the model to that specific dataset.  We can create increasingly complex models that can serve to increase accuracy in training scores, but it may not be optimal once another sample is introduced to the model.  In order to observe how the model can perform in the real world (where labels are not readily available), we must test our trained model against unseen data.  This method of validation will allow us to compare how performant different models are against new data, and there are many different methods for splitting data into training and validation sets.
